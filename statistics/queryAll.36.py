...
url0 = 'http://biosoda-prod.cloudlab.zhaw.ch:8080/rdf4j-server/repositories/bgeelight'
...
addlimitsnumber = 10
addlimits = 'LIMIT ' + str(addlimitsnumber)
addlimits10  = 'LIMIT ' + str(addlimitsnumber * 10)
...
printwrite('LIMITS10: ' + addlimits10)
printwrite('url0, 1: \n' + url0 + '\n' + url1 + ' (' + url1_title + ')')
...
with open(targetfile) as jsondata:
    ...
    for onequery in data['questions']:
        ...
        tmpSPARQL = tmpSPARQL.replace(separator + 'innerlimit10' + separator, ' ' + addlimits10 + ' ')
...
printwrite('### this statistics file ist created by bioSODA queryAll.py (https://github.com/biosoda/bioquery/tree/master/statistics) - created at ' + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
